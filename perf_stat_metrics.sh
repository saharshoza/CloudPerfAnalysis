...
fileList=(cpi.out cpi.out branch.out branch.out l1dcache_loads.out l1dcache_loads.out l1dcache_stores.out l1dcache_stores.out dtlb_loads.out dtlb_loads.out dtlb_stores.out dtlb_stores.out llc.out llc.out)
lenList=${#fileList[@]}
for i in {0..12}
do
	echo ${grepList[i]}
